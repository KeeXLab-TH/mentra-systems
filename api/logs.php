...
<body class="text-slate-700">
                    ...
                    <tbody id="logsList" class="text-sm divide-y divide-slate-100">
                        <tr>
                            <td colspan="5" class="text-center py-12 text-slate-400"><i
                                    class="fa-solid fa-spinner fa-spin text-indigo-500 text-xl"></i> กำลังโหลดประวัติ...
                            </td>
                        </tr>
                    </tbody>
                </table>
    ...
    <script type="module">
        ...
        const getActionBadge = (action) => {
            if (action === 'เพิ่มรายการ') return '<span class="px-2.5 py-1 rounded bg-green-100 text-green-700 font-bold text-[10px]"><i class="fa-solid fa-plus mr-1"></i> เพิ่ม</span>';
            if (action === 'อัปเดตรายการ') return '<span class="px-2.5 py-1 rounded bg-blue-100 text-blue-700 font-bold text-[10px]"><i class="fa-solid fa-pen mr-1"></i> แก้ไข</span>';
            if (action === 'ลบรายการ') return '<span class="px-2.5 py-1 rounded bg-red-100 text-red-700 font-bold text-[10px]"><i class="fa-solid fa-trash mr-1"></i> ลบ</span>';
            if (action === 'อัปเดตสถานะ') return '<span class="px-2.5 py-1 rounded bg-purple-100 text-purple-700 font-bold text-[10px]"><i class="fa-solid fa-rotate mr-1"></i> สถานะ</span>';
            return `<span class="px-2.5 py-1 rounded bg-slate-100 text-slate-700 font-bold text-[10px]">${escapeHtml(action)}</span>`;
        };

        onAuthStateChanged(auth, (user) => {
            if (user) {
                const q = query(getLogsRef(), orderBy('timestamp', 'desc'), limit(100));
                onSnapshot(q, (snap) => {
                    let html = '';
                    if (snap.empty) {
                        html = '<tr><td colspan="5" class="text-center py-12 text-slate-400"><i class="fa-solid fa-history text-xl block mb-2 opacity-50"></i> ยังไม่มีประวัติการทำรายการ</td></tr>';
                    } else {
                        snap.forEach(doc => {
                            const data = doc.data();
        ...
        if (urlProject) {
            const links = document.querySelectorAll('a[href^="bom.php"], a[href^="drawings.php"], a[href^="calculator.php"]');
            links.forEach(link => {
                const baseHref = link.getAttribute('href').split('?')[0];
                link.setAttribute('href', baseHref + '?project=' + urlProject);
            });
        }
    </script>
    </main>
